...
def search(l, s, sp, ep):
		print "\nBinary Searching........."
		slist = l
		searchValue = s  
		startSearchParameter = sp
		endSearchParameter = ep
		middleValue = (startSearchParameter+endSearchParameter/2)
		if searchValue == slist[middleValue]:
			return middleValue
		else:	
			if searchValue < slist[middleValue]:
				endSearchParameter = middleValue - 1
				return search(slist, searchValue, startSearchParameter, endSearchParameter)
			else:
...
def binarySearch(l, s):
	...
	sValue = s
	message = search(list, sValue, startParameter, endParameter)
	if message == -1:
		print "Search Value not found.... %d does not appear to be in the given list" %sValue
	else:
		print "\nSearch value %d found at position %d" % (sValue, message)
